...
function openMailapp(e) {
  ...
  return (
    <a href={`mailto:${email}?subject=${subject}&body=${body}`}>
      Click here to open the default mail application
    </a>
  );
}
...
const ForgotPasswordSentSuccess = () => {
    ...
    try {
      if (email !== '') {
        ...
        const response = await axios.put(
          `${proxy}/api/users/update/password/forgot`,
          data
        );
        alert('Email sent');
      }
  ...
  return (
    <>
      <Helmet>
        <title>4EverStay | Token sent</title>
        <meta
          name="description"
          content="Password reset token was sent sucessfully"
        />
      </Helmet>
      <NotificationBar />
      <Container>
        <FormContainer style={{ width: '240px', gap: '24px' }}>
          ...
          <Subtitle style={{ marginTop: '-16px' }}>
            We have sent you an email with a link to {email}
          </Subtitle>
          <Button>
            <a
              href={`mailto:${email}?subject=${'Check your mail'}&body=${'Reset Password Link was sent to you'}`}
            >
              Open email app
            </a>
          </Button>
          <ResendMail>
            ...
            <ResendLink onClick={resendMail}>Click to resend</ResendLink>
          </ResendMail>
          <BackButton>
            <Link to="/login">
              <IoArrowBackOutline />
