...
function HostelSearchResultPage() {
  ...
  const [url, setUrl] = useState(
    `${proxy}/api/hostels?name=${name}&room_types=${
      options === 'any'
        ? 'one_seater,two_seater,three_seater,four_seater'
        : options + '_seater'
    }&longitude=${destination.longitude || ''}&latitude=${
  ...
  return (
    <>
      <Helmet>
        <title>4EverStay | Search</title>
        <meta
          name="description"
          content="Search results of hostels with specific location/name/seater/price etc."
        />
      </Helmet>
      <NavAndSidebar />
      <Container>
        <Wrapper>
          ...
          <ResultSection>
            ...
            {loading
              ? 'Loading text here please...'
              : toggleMap && <SearchMap hostels={data} />}
          </ResultSection>
        </Wrapper>
      </Container>
    </>
